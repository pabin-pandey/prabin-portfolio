...
const contactLinks = [
  {
    type:  "Email",
    value: "prabin.pandey@temple.edu",
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100" style={{ fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <main className="pt-24 pb-20 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* ── Response info strip ── */}
        <div
          className="flex flex-wrap gap-0 mb-10 rounded-xl overflow-hidden"
        ...
        >
          {responseInfo.map((r, i) => (
            <div
              ...
              style={{
                borderRight: i < responseInfo.length - 1 ? "1px solid rgba(255,255,255,0.06)" : "none",
              }}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
              ...
              return (
                <Wrapper
                  ...
                  onMouseEnter={(e) => {
                    if (link.href) {
                      ...
                      el.style.boxShadow = `0 0 20px ${link.color}12`;
                    }
                  }}
                  onMouseLeave={(e) => {
                    if (link.href) {
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-600 font-medium mb-0.5">{link.type}</p>
                    <p
                      className="text-sm font-semibold truncate transition-colors duration-200"
                      style={{ color: link.href ? "#e5e7eb" : "#d1d5db" }}
                    >
                      {link.value}
                    </p>
                  </div>
                  {link.href && (
                    <span
                      className="text-gray-600 text-sm transition-all duration-200 group-hover:text-gray-400"
                      style={{ transform: "translateX(0)", transition: "transform 0.2s ease, color 0.2s ease" }}
                    >
                      →
                    </span>
                  )}
            ...
            {/* Availability badge */}
            <div
              className="relative rounded-xl p-4 overflow-hidden mt-2"
              style={{
