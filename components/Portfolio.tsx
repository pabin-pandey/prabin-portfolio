...
const DEFAULT_CONTENT = {
  ...
  about: {
    ...
    experience: [
      { co: "Sethi Clarity Advisers", role: "Associate Financial Consultant", period: "September 2025 – December 2025", pts: ["Developed multi-scenario valuation models in Excel and Python to analyze portfolio allocations, stress-test return assumptions across market environments, and benchmark risk-adjusted performance against targets — supporting data-driven investment decisions.", "Built AI-integrated analytical tools to simulate financial scenarios, identify valuation discrepancies across investment positions, and monitor portfolio performance against benchmarks; improved reporting accuracy and consistency across multi-scenario outputs.", "Designed automated financial reporting dashboards with real-time data pipelines and AI-driven analytics, streamlining investment reporting workflows and enabling continuous performance monitoring for senior stakeholder review."] },
      ...
      { co: "Global IME Bank", role: "Customer Relations Intern", period: "March 2024 – May 2024", pts: ["Prepared collateral valuation reports and credit risk summaries assessing fixed income pricing, debt service coverage ratios, and risk-adjusted lending terms to support management decisions on capital allocation; maintained audit-ready client financial records.", "Analyzed client financial profiles using cash flow modeling and ratio analysis, translating complex financial data into structured, risk-appropriate investment recommendations across diverse client engagements."] }
    ],
    skills: [
  ...
  blog: [
    { id: "b2", title: "When the Output Looks Right but Isn't", date: "2026-02-25", excerpt: "AI tools produce outputs that are fluent, structured, and internally consistent. In financial analysis, that introduces a specific kind of risk — convincing is not the same as correct.", content: "###Brief Context|Working through a series of applied AI assignments in a graduate finance program, the same tension kept surfacing. AI tools produce outputs that are fluent, structured, and internally consistent. In most fields, that's useful. In financial analysis, it introduces a specific kind of risk. Convincing is not the same as correct.|###Central Question|The question isn't whether AI is useful in finance. It clearly is. The harder question is: at what point does a well-formed AI output become a liability rather than an asset?|###Reasoning|The first observation came from understanding what AI models actually do. They predict the next word based on patterns in training data. They don't verify claims. So when a model states that a fund \"is expected to underperform the benchmark,\" that sentence reads like an analytical conclusion. But it may have no supporting evidence. The model generated it because it fits the pattern — not because it's true.|This matters more in finance than in most fields. A DCF model built with AI-generated discount rates or growth assumptions can produce a complete, internally consistent output — and still be wrong. The numbers will hang together. The error won't be visible until something depends on it.|The second observation was about trust. One of the readings introduced the concept of \"narrative fluency\" — newer AI models don't just produce coherent responses, they mask incorrect information more convincingly than older models did. The output doesn't just sound right. It sounds authoritative. That's the actual problem.|Thinking through DCF workflows made this concrete. Some stages — discounting, formula links, equity value calculations — can be tested mechanically. A formula either references the right cell or it doesn't. The error is structural and identifiable.|But other stages — defining scope, designing scenarios, selecting benchmarks — involve judgment. No formula determines whether a growth assumption is appropriate for a specific business in a specific market. Only understanding the business does.|The implication is that AI works better as a structural tool than as an analytical one. It can build the scaffolding. It can write the code that computes the output. Humans still need to decide what goes inside the model and whether the output means what it appears to mean.|###Key Observations|Narrative validation matters more than tonal analysis. In MD&A work, the most important check wasn't whether management sounded confident or cautious. It was whether the narrative was consistent with the audited financial statements. Management writes the MD&A. Auditors verify the numbers. Those are different processes with different incentives. When they diverge, the audited numbers take priority — always. Skipping that reconciliation step makes every downstream analysis unreliable, regardless of how sophisticated the AI output looks.|Overconfidence in simulation outputs is easy to miss. One annotated AI output claimed that tracking error \"exhibits notable consistency across simulated paths.\" That sentence sounds like an observation. It isn't. Consistency across simulated paths says nothing about what happens in actual markets. The sentence was generated because it fits the expected language of simulation analysis — not because it's a meaningful finding.|Temperature settings shape how reliable AI outputs are. At low temperature, the model produces focused, repeatable responses suited to tasks that require accuracy — summaries, translations, formula review. At high temperature, the model explores more but varies more, which has a place in brainstorming or early scenario generation. For financial analysis where outputs will be used as inputs, low temperature is generally the safer default.|###Practical Meaning|The governance frameworks that major financial institutions have built around AI aren't excessive caution. They're a practical response to a clear problem: AI cannot be accountable. Humans are. Requiring model registration, independent validation, and human sign-off before AI outputs are used in client-facing decisions puts accountability exactly where it needs to be.|For an individual analyst, the practical implication is simpler. Treat AI outputs as a first draft requiring review — not a conclusion requiring formatting. The review isn't about distrust. It's about recognizing where the model's confidence comes from and whether that confidence is earned.|###Limits or Uncertainty|These observations came from academic assignments, not live production environments. Real financial workflows involve data quality issues, time pressure, team dynamics, and institutional constraints that change how AI tools are used in practice. What looks like a clear validation step in a structured exercise may be harder to apply consistently when a deadline is close and the output looks correct.|There's also an open question about where exactly AI-assisted analysis ends and AI-generated analysis begins. In most of these assignments, AI helped build code, structure outputs, and surface patterns — but judgment calls stayed with the analyst. Whether that balance holds as AI tools become more capable and more embedded in standard workflows is genuinely unclear.|###Closing Reflection|The most durable insight from working through these materials wasn't about what AI can do. It was about what makes AI specifically risky: not that it produces wrong outputs, but that it produces wrong outputs that look right. That's a different problem than accuracy. It's a credibility problem. Better models won't solve it. Better practices might.", published: true, tags: ["AI", "Finance", "Governance"] }
  ],
  testimonials: [
...
function Blog({ c, d }) {
  ...
  if (sel) {
    const blocks = sel.content.split('|').map((b: string) => b.trim()).filter(Boolean);
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <button onClick={() => setSel(null)} className={`flex items-center gap-2 text-sm font-medium mb-12 transition-colors ${d ? "text-gray-400 hover:text-indigo-400" : "text-gray-500 hover:text-indigo-600"}`}>
          ← Back to Blog
        </button>
        <div className="flex items-center gap-3 mb-4">
          <span className={`text-xs font-medium ${d ? "text-gray-500" : "text-gray-400"}`}>{sel.date}</span>
          <span className={`w-px h-3 ${d ? "bg-gray-800" : "bg-gray-200"}`} />
          {sel.tags.map((t: string) => <span key={t} className={`text-[11px] font-semibold px-2 py-0.5 rounded-lg ${d ? "bg-indigo-500/10 text-indigo-400 border border-indigo-500/20" : "bg-indigo-50 text-indigo-600"}`}>{t}</span>)}
        </div>
        <h1 className={`text-3xl sm:text-4xl font-black tracking-tight mb-10 ${d ? "text-white" : "text-gray-900"}`}>{sel.title}</h1>
        <div className={`border-t mb-10 ${d ? "border-white/[0.07]" : "border-gray-200"}`} />
        <div className="space-y-5">
          {blocks.map((block: string, i: number) => {
            if (block.startsWith('###')) {
              return <h2 key={i} className={`text-sm font-bold tracking-widest uppercase pt-6 ${d ? "text-indigo-400" : "text-indigo-600"}`}>{block.replace('###', '').trim()}</h2>;
            }
            return <p key={i} className={`text-sm leading-relaxed ${d ? "text-gray-300" : "text-gray-700"}`}>{block}</p>;
          })}
        </div>
        <div className={`border-t mt-12 pt-6 ${d ? "border-white/[0.07]" : "border-gray-200"}`}>
          <p className={`text-xs ${d ? "text-gray-500" : "text-gray-400"}`}>Published {sel.date} · Prabin Pandey</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
      ...
      {pub.length === 0 ? (
        ...
        <article key={post.id} onClick={() => setSel(post)} className={`p-6 rounded-2xl border mb-5 hover-lift transition-all duration-200 cursor-pointer ${d ? "bg-gray-900/60 border-white/[0.07] hover:border-indigo-500/20" : "bg-white border-gray-200 shadow-sm hover:shadow-md"}`}>
          ...
          <p className={`text-xs font-semibold mt-4 ${d ? "text-indigo-400" : "text-indigo-600"}`}>Read more →</p>
        </article>
      ))}
    </div>
  );
