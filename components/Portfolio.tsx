...
const DEFAULT_CONTENT = {
  ...
  blog: [
    { id: "b2", title: "When the Output Looks Right but Isn't", date: "2026-02-25", excerpt: "AI tools produce outputs that are fluent, structured, and internally consistent. In financial analysis, that introduces a specific kind of risk — convincing is not the same as correct.", content: "Brief Context: Working through a series of applied AI assignments in a graduate finance program, the same tension kept surfacing. AI tools produce outputs that are fluent, structured, and internally consistent. In most fields, that's useful. In financial analysis, it introduces a specific kind of risk. Convincing is not the same as correct. Central Question: The question isn't whether AI is useful in finance. It clearly is. The harder question is: at what point does a well-formed AI output become a liability rather than an asset? Reasoning: AI models predict the next word based on patterns in training data. They don't verify claims. So when a model states that a fund \"is expected to underperform the benchmark,\" that sentence reads like an analytical conclusion. But it may have no supporting evidence. A DCF model built with AI-generated discount rates or growth assumptions can produce a complete, internally consistent output — and still be wrong. One of the readings introduced the concept of \"narrative fluency\" — newer AI models mask incorrect information more convincingly than older models did. The output doesn't just sound right. It sounds authoritative. Key Observations: Narrative validation matters more than tonal analysis — when MD&A diverges from audited financials, the audited numbers take priority. One annotated AI output claimed that tracking error \"exhibits notable consistency across simulated paths\" — that sentence sounds like an observation, but it isn't. Low temperature is generally the safer default for financial analysis. Practical Meaning: AI cannot be accountable. Humans are. Treat AI outputs as a first draft requiring review — not a conclusion requiring formatting. Closing: The most durable insight is that AI produces wrong outputs that look right. That's a credibility problem. Better models won't solve it. Better practices might.", published: true, tags: ["AI", "Finance", "Governance"] }
  ],
  testimonials: [
